...
jQuery.extend = jQuery.fn.extend = function() {
	...
	for ( ; i < length; i++ ) {
		...
		if ( ( options = arguments[ i ] ) != null ) {
			...
			// Extend the base_template object
			for ( name in options ) {
				copy = options[ name ];
	...
	jQuery.event.add( el, type, {
		...
		handler: function( event ) {
			...
			if ( ( event.isTrigger & 1 ) && this[ type ] ) {
				...
				// bubbling surrogate propagates *after* the non-bubbling base_template), but that seems
...
jQuery.parseHTML = function( data, context, keepScripts ) {
	...
	if ( !context ) {
		...
		if ( support.createHTMLDocument ) {
			...
			// Set the base_template href for the created document
